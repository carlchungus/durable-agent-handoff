...
type transcriptCase struct {
	...
	Claim             supervisor.WorkerResult `json:"claim"`
...
func TestRealTranscriptFixturesAreWellFormed(t *testing.T) {
	for _, fixture := range loadTranscriptCases(t) {
		if fixture.Name == "" || fixture.Source == "" || fixture.Goal == "" || fixture.Prompt == "" || fixture.Claim.Status == "" || fixture.Claim.Summary == "" || fixture.ExpectedOutcome == "" {
		...
		t.Run(string(mode), func(t *testing.T) {
			...
			for _, fixture := range loadTranscriptCases(t) {
				for run := 1; run <= 5; run++ {
					...
					decision, err := client.Evaluate(ctx, Request{Model: DefaultModel, Goal: fixture.Goal, Prompt: fixture.Prompt, SupervisorContext: fixture.SupervisorContext, Claim: fixture.Claim})
					cancel()
					if err != nil {
...
func loadTranscriptCases(t *testing.T) []transcriptCase {
	t.Helper()
	raw, err := os.ReadFile(filepath.Join("testdata", "real_transcript_claims.json"))
	if err != nil {
		t.Fatal(err)
	}
